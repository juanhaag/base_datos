...
private:
    ...
    void openDb(const std::string& dbPath) {
        ...
        // Depuración: Mostrar la ruta de la base de datos
        std::cout << "Opening database at path: " << dbPath << std::endl;
...
public:
    ...
    void setConfFile(const std::string& confFilePath) override {
        ...
        // Depuración: Verificar que se obtiene la ruta correcta
        std::cout << "Database path from configuration: " << dbPath << std::endl;
