...
void initializeDatabase(std::shared_ptr<DatabaseHandler> dbHandler) {
    std::ifstream schemaFile("../db/create_tables.sql");  // Ruta relativa ajustada
    if (!schemaFile.is_open()) {
        std::cerr << "Error: Could not open schema file at ../db/create_tables.sql" << std::endl;
        return;
    }
    std::string schema((std::istreambuf_iterator<char>(schemaFile)), std::istreambuf_iterator<char>());

    // Depuración: imprime el contenido del esquema
    std::cout << "Schema read from file:" << std::endl << schema << std::endl;
